...
RESULTS_DIR="${WORKDIR}/meme_results_${DATASET_TAG}"
mkdir -p "$RESULTS_DIR"
...
# Host-side vars
WORKDIR="$(pwd)"
RESULTS_DIR="${WORKDIR}/meme_results_arc_3pUTR"
BFILE_DIR="${RESULTS_DIR}/background_models"

# Create the folder on the host so Docker sees it
mkdir -p "$BFILE_DIR"

# Container-visible paths
RESULTS_DIR_IN="/data/$(basename "$RESULTS_DIR")"
BFILE_DIR_IN="${RESULTS_DIR_IN}/background_models"

# Order-1 model
docker run --rm -u "$(id -u)":"$(id -g)" \
  -v "${WORKDIR}:/data" "$DOCKER_IMAGE" \
  fasta-get-markov -dna -m 1 \
  "/data/${BACKGROUND_FASTA##*/}" \
  "${BFILE_DIR_IN}/bfile_order1.meme"

# Order-2 model
docker run --rm -u "$(id -u)":"$(id -g)" \
  -v "${WORKDIR}:/data" "$DOCKER_IMAGE" \
  fasta-get-markov -dna -m 2 \
  "/data/${BACKGROUND_FASTA##*/}" \
  "${BFILE_DIR_IN}/bfile_order2.meme"

# Choose which background file to use for the runs below (order-2 is stricter)
BFILE_CHOICE="${BFILE_DIR_IN}/bfile_order2.meme"
...
#############################################################
OUT1_IN="${RESULTS_DIR_IN}/meme_zoops_w6-12_seed11"

docker run --rm -u "$(id -u)":"$(id -g)" \
  -v "${WORKDIR}:/data" memesuite/memesuite:latest \
  meme "/data/${TARGET_FASTA##*/}" \
  -mod zoops \
  -nmotifs 10 \
  -minw 6 -maxw 12 \
  -bfile "${BFILE_DIR_IN}/bfile_order2.meme" \
  -evt 0.05 \
  -minsites 10 \
  -maxsites 300 \
  -seed 11 \
...
#############################################################
OUT2_IN="${RESULTS_DIR_IN}/meme_anr_w6-12_seed22"
...
#############################################################
OUT2_IN="${RESULTS_DIR_IN}/meme_zoops_w5-8_evt0.01_seed33"
