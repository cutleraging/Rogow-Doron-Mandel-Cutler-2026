...
docker run --rm -u "$(id -u)":"$(id -g)" -v "${WORKDIR}:${DATA_IN}" "$DOCKER_IMAGE" \
  fasta-get-markov -dna -m 0 "${DATA_IN}/${BACKGROUND_FASTA}" > "${BFILE_DIR}/bfile_order0.meme"

docker run --rm -u "$(id -u)":"$(id -g)" -v "${WORKDIR}:${DATA_IN}" "$DOCKER_IMAGE" \
  fasta-get-markov -dna -m 1 "${DATA_IN}/${BACKGROUND_FASTA}" > "${BFILE_DIR}/bfile_order1.meme"

docker run --rm -u "$(id -u)":"$(id -g)" -v "${WORKDIR}:${DATA_IN}" "$DOCKER_IMAGE" \
  fasta-get-markov -dna -m 2 "${DATA_IN}/${BACKGROUND_FASTA}" > "${BFILE_DIR}/bfile_order2.meme"

# Choose which bfile to use
BFILE_CHOICE_IN="${BFILE_DIR_IN}/bfile_order0.meme"
