...
def plot_publication_summary(
    ...
    if "eligible_for_summary" in observed_df.columns:
        observed_df = observed_df[observed_df["eligible_for_summary"]].copy()
    if "eligible_for_summary" in null_mean_df.columns:
        null_mean_df = null_mean_df[null_mean_df["eligible_for_summary"]].copy()
...
def main() -> None:
    ...
    parser.add_argument("--min-total-puncta-per-bin", type=int, default=0)
    ...
    eligible_by_bin = {bin_spec.name: [] for bin_spec in bin_specs}
    ...
    for image_id in shared_images:
        ...
        for bin_spec in bin_specs:
            ...
            eligible = observed["total_puncta"] >= args.min_total_puncta_per_bin
            ...
            eligible_by_bin[bin_spec.name].append(eligible)
            ...
            null_mean_rows.append(
                {
                    "image_id": image_id,
                    "bin": bin_name,
                    "eligible_for_summary": eligible_by_bin[bin_name][-1],
    ...
    summary_rows = []
    for bin_spec in bin_specs:
        observed = observed_df.loc[
            (observed_df["bin"] == bin_spec.name) & observed_df["eligible_for_summary"],
            "coassoc_freq",
        ].to_numpy(dtype=float)
        per_image_null = np.asarray(null_metric_values[bin_spec.name], dtype=float)
        eligible_mask = np.asarray(eligible_by_bin[bin_spec.name], dtype=bool)
        if per_image_null.size and eligible_mask.any():
            group_null = np.mean(per_image_null[eligible_mask], axis=0)
        else:
